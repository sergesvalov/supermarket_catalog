...
export function ProductCard(p) {
    ...
    const shopName = p.shop ? escapeHtml(p.shop.name) : '<span class="text-muted fst-italic">Без магазина</span>';
    ...
    <div class="card mb-2 p-3 border-0 shadow-sm">
        <div class="d-flex justify-content-between align-items-center">
            <div class="flex-grow-1">
                <h5 class="m-0">${escapeHtml(p.name)}</h5>
    ...
    <div class="col-md-6 mb-3">
        <div class="card h-100 shadow-sm list-card cursor-pointer" data-id="${list.id}">
            <div class="card-body d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="card-title mb-1">${escapeHtml(list.name)}</h5>
...
export function ShoppingListItemRow(item) {
    ...
    const shopName = p.shop ? escapeHtml(p.shop.name) : '???';
    ...
    <li class="list-group-item d-flex align-items-center justify-content-between">
        <div class="d-flex align-items-center gap-3">
            ...
            <div class="${strike}">
                <div class="fw-bold">${escapeHtml(p.name)} <span class="badge bg-secondary rounded-pill ms-1">x${item.quantity}</span></div>
...
export function ProductSearchItem(p) {
    const shopName = p.shop ? escapeHtml(p.shop.name) : 'Без магазина';
    return `
